%% 20.380 modeling
k =[2.614e-4, 1.01e14, 1e6, .27, 9.33, 1e5, 2.306e-9,3600];
...
C=['-k','-b','-r','-g','-y'];
for i=1:5
    k_iter=[2.614e-4, 1.01e14, kon_i_vals(i), .27, 1.14, 1e5, 2.306e-9,3600];
...
hold all
for i=1:4
    k_iter=[2.614e-4, 1.01e14, 1e-6, .27, 1.14, 1e5, 2.306e-9,delivery_times(i)];
    [t3,y3]=ode15s(@model,t_span_3, y0, ode_options, k_iter);
    subplot(2,1,1)
    hold on
    plot(t3,y3(:,4),C(i))
    xlabel('Time (s)')
